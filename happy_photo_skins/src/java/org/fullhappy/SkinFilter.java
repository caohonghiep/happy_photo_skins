 ...
 * @author AAA
 */
@WebFilter(filterName = "SkinFilter", urlPatterns = {"/*"})
public class SkinFilter implements Filter {
    ...
    private void doBeforeProcessing(ServletRequest request, ServletResponse response)
        ...
        if (debug) {
            log("SkinFilter:DoBeforeProcessing");
        }
    }
    ...
    private void doAfterProcessing(ServletRequest request, ServletResponse response)
            throws IOException, ServletException {
        if (debug) {
            log("SkinFilter:DoAfterProcessing");
        }
    }
    ...
    public void doFilter(ServletRequest request, ServletResponse response,
        ...
        if (debug) {
            log("SkinFilter:doFilter()");
        }

        doBeforeProcessing(request, response);

        //http://www.mysamplecode.com/2012/02/access-control-origin-not-allowed.html
        ...
        if (incomingURLs.indexOf(clientOrigin) >= 0) {
    ...
    public void init(FilterConfig filterConfig) {
        ...
        if (filterConfig != null) {
            if (debug) {
                log("SkinFilter:Initializing filter");
            }
        }
    }
    ...
    public String toString() {
        ...
        StringBuffer sb = new StringBuffer("SkinFilter(");
